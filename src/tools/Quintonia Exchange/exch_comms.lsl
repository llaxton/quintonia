...
// Server URL (sent from mian script)
string BASEURL = "";
...
list exchData = [];
// exchData [exchangeID, owner, farmName, regionName, slurl, position, rating, exchangeType, accessMode, opMode, avatars, totalSold, totalPurchased]
...
default
{
    ...
    state_entry()
    {
        ...
        llPassTouches(1);
        regionName = llGetRegionName();
        slurl = osGetGridHomeURI() +":" + osGetGridName();
        position = llGetPos(); 
        status = "init";
