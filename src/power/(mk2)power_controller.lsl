...
float   VERSION = 5.3;     // 6 January 2022
...
displayData(integer forceUpdate)
{
    ...
    {
        lastRefresh = ts;
        string body = "width:512,height:512,aplha:FALSE,bgcolour:darkslategray";    
        ...
        draw = osDrawRectangle(draw, 510,500);
        ...
        if (i != -1)
        {
           statusText += "\n" + fixedStrLen(SF_KWH, 12) + "\t  -\t  -\t  " +fixedStrLen(llList2String(generators, i-1), 3) + "\n";
        }
        ...
        if (master == TRUE)
        {
            ...
            if (energy <1000)
            {
                statusText += fixedPrecision(energy, 1) + " " + TXT_KWH + "\n";
                llSetText(TXT_HEADING + "\n" + TXT_TOTAL_ENERGY + " " + fixedPrecision(energy, 1) + " " + TXT_KWH + "\n" +tmpStr, <1,1,1>, 1.0);
            }
            else
            {
                statusText += fixedPrecision((energy/1000), 2) + " " + TXT_MWH + "\n";
                llSetText(TXT_HEADING + "\n" + TXT_TOTAL_ENERGY + " " + fixedPrecision((energy/1000), 2)  + " " + TXT_MWH  + "\n" +tmpStr, <1,1,1>, 1.0);
            }
        }
