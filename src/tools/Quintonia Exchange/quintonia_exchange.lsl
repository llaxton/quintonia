...
{  
    ...
    psys(userToPay);
    llMessageLinked(LINK_SET, 1, "CMD_INIT|" +PASSWORD +"|" +(string)((integer)(VERSION*10)) +"|" +(string)RSTATE +"|" +ExchangeID +"|" +(string)joomlaID +"|" +BASEURL, "");  
    llSetTimerEvent(60);
}
...
broadcast_data()
{
    ...
    string cmdList = "EXCH_VALUES|" +(string)((integer)(VERSION*10)) +"|" +(string)RSTATE +"|" +ExchangeID +"|" +exchangeType +"|" +(string)offset +"|" +(string)allowRegister +"|" +farmName +"|" +(string)accessMode +"|" +(string)opMode +"|" +joomlaID;
